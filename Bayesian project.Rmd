...
sigma2_hat <- RSS / (n - p)      # unbiased; for pure MLE you’d divide by n, but this is fine
...
# Assumes: y, x_1, x_2, alphamean, alphavar, beta_1mean, beta_1var,
#          beta_2mean, beta_2var, taushape, tausscale are defined.
...
# posterior means (after discarding e.g. first 5000 as burn-in)
burnin = 10000
...
```{r}
# assume gibbs_chain has columns: alpha,beta1,beta2,tau
...
for (b in 1:B) {
  ...
  # 2) run your Gibbs sampler on this dataset
...
```{r}
# Assume: draws is N x 4 with columns: alpha,beta1,beta2,tau
...
# prior precision matrix for beta (inverse variances)
B0 <- diag(c(1/alphavar, 1/beta_1var, 1/beta_2var))

# prior for sigma^2: sigma^2 ~ Inv-Gamma(c0/2, d0/2)
# which corresponds to tau ~ Gamma(c0/2, d0/2)
c0 <- 2 * taushape
d0 <- 2 * 1/taushape #taurate

# How many post–burn-in draws do you want from MCMCpack?
niter = 50000
burnin = 10000
...
colnames(post_mcmc)
# likely: "(Intercept)", "x_1", "x_2", "sigma2"

tau_mcmc <- 1 / post_mcmc[,"sigma2"]

# Create a matrix with same parameter order as your Gibbs
...
# 1) Your Gibbs (post burn-in)
post_gibbs <- gibbs_chain[(burnin+1):nrow(gibbs_chain), ]
colnames(post_gibbs) <- c("alpha","beta1","beta2","tau")
gibbs_means <- colMeans(post_gibbs)

# 2) Importance sampling (IS resampled draws)
post_IS <- cbind(
  alpha = alpha_list,
  beta1 = beta1_list,
  beta2 = beta2_list,
